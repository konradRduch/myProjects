...
class Nauczyciela :public Ekran
{
...
public:
    void wyswietl() {
        std::wcin.ignore();
        OdczytPliku listaNauczycieli("C:\\Users\\HP\\Desktop\\sem2_projekt\\dokument.txt");
        std::wstring imie=L"";
        listaNauczycieli.pokazOdczytaneDane();
        std::wcout << L"Wybierz nauczyciela z powy¿szej listy\n";
        getline(std::wcin, imie);
        ...
        ListaZStruktura lm = l.tablicaZPodobneImie(imie);
        ...
        for (int i = 1;i < lm.rozmiarListy();i++) {
            ...
            std::vector<std::wstring>li{ lu.przedmiot,lu.kierunek,lu.sala };
            t1.wstaw(lu, li);
        }
