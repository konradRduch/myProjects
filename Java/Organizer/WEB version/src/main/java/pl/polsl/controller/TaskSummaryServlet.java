 ...
 * @since f4
 */
@WebServlet(name = "TaskSummaryServlet", urlPatterns = {"/TaskSummaryServlet"})
public class TaskSummaryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            List<Object> taskCompletedList = new ArrayList();
            taskCompletedList = dbManager.findObjects("SELECT t FROM TaskEntity t WHERE t.done = true");
            Long completedTaskCount = (Long) dbManager.findObjects("SELECT COUNT(t) FROM TaskEntity t WHERE t.done = true").get(0);

            List<Object> taskUncompletedList = new ArrayList();
            taskUncompletedList = dbManager.findObjects("SELECT t FROM TaskEntity t WHERE t.done = false");
            Long uncompletedTaskCount = (Long) dbManager.findObjects("SELECT COUNT(t) FROM TaskEntity t WHERE t.done = false").get(0);
            ...
            out.println("COMPLETED TASKS (" + completedTaskCount + ")");
            out.println("<br>");
            for (Object obj : taskCompletedList) {
                if (obj instanceof TaskEntity) {
                    TaskEntity taskEntity = (TaskEntity) obj;
                    out.println("<p>ID: " + taskEntity.getId() + "</p>");
            ...
            out.println("<br>");
            out.println("UNCOMPLETED TASKS (" + uncompletedTaskCount + ")");
            out.println("<br>");
            for (Object obj : taskUncompletedList) {
                if (obj instanceof TaskEntity) {
                    TaskEntity taskEntity = (TaskEntity) obj;
                    out.println("<p>ID: " + taskEntity.getId() + "</p>");
                    out.println("<p>Description: " + taskEntity.getDescription() + "</p>");
